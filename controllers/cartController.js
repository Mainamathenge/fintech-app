...
exports.createCart = catchAsync(async (req, res, next) => {
  ...
  if (cart) {
    ...
    next(new AppError('Cart could not be created', 403));
  }
});
...
exports.checkout = catchAsync(async (req, res) => {
  ...
  const response = await mpesaC2bService.initiatePayment(cart.bill, number);
...
exports.payment = catchAsync(async (req, res, next) => {
  ...
  const response = await order_payment.postStk(amount, phone);
